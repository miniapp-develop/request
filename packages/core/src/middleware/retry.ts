 ...
 *
 * 终态判定：库源错误（RequestError）一律不重试（超时/取消/不支持都是终态）；
 * 原生错误交由 engine.isTransientError 决定（缺省时默认可重试）。
 */
export async function runWithRetry(
    ...
    for (let attempt = 0; ; attempt++) {
        try {
            ...
            const isTransient = typeof ctx.engine.isTransientError === 'function'
                ? ctx.engine.isTransientError(err)
                : true;
