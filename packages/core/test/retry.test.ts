...
function ctx(engine?: Partial<RequestEngine>): RequestContext {
    return {
        method: 'request',
        options: {},
        canonical: {},
        ext: {},
        engine: { ...engine } as RequestEngine,
        result: undefined,
        state: {}
    };
}
...
describe('runWithRetry', () => {
    ...
    it.each(['TIMEOUT', 'CANCELLED', 'UNSUPPORTED'] as const)('库源错误 %s 终态不重试', async (code) => {
        const exec = jest.fn(async () => {
            throw new RequestError(code, 'x');
        });
        await expect(runWithRetry(ctx(), exec, { retry: 3, delay: instantDelay })).rejects.toMatchObject({ code });
        expect(exec).toHaveBeenCalledTimes(1);
    });

    it('engine.isTransientError 返回 false 时不重试', async () => {
        const exec = jest.fn(async () => {
            throw { errMsg: 'request:fail timeout' };
        });
        await expect(
            runWithRetry(ctx({ isTransientError: () => false }), exec, { retry: 3, delay: instantDelay })
        ).rejects.toEqual({ errMsg: 'request:fail timeout' });
        expect(exec).toHaveBeenCalledTimes(1);
    });

    it('engine.isTransientError 返回 true 时重试', async () => {
        let n = 0;
        const exec = jest.fn(async () => {
