...
/** 假 engine：记录入参，返回一个填好的 Response（平台无关，测 facade 归一逻辑）。 */
function mockEngine() {
    const calls: any[] = [];
    const engine = {
...
describe('createRequest facade (my)', () => {
    it('无 engine 时 handle 抛 No engine found（无 my 环境）', async () => {
    ...
    it('headers/header 归一为 canonical headers（支付宝原生 headers → 双写 headers 赢）', async () => {
        const { engine, calls } = mockEngine();
        const req = createRequest(engine);
        await req({ url: 'u', headers: { a: '1' } });
        expect(calls[0].args.headers).toEqual({ a: '1' });
        expect(calls[0].args).not.toHaveProperty('header');
        // 支付宝原生字段是 headers（无 header）：双写时 headers 赢（与 wx/tt 的 header 赢相反）
    ...
    it('createRequest() 缺省懒绑 my 全局（设置 global.my）', async () => {
        // 支付宝 success res = { data, status, headers }
        const fakeVendor = {
            request(option: any) {
                option.success({ data: 'from-my', status: 200, headers: {} });
                return { abort() {} };
            }
        };
        (global as any).my = fakeVendor;
        try {
            const req = createRequest();
            expect(req.engine).not.toBeNull();
            const res = await req({ url: 'u' });
            expect(res.data).toBe('from-my');
        } finally {
            delete (global as any).my;
        }
    });

    it('createRequest() 无 my 无 engine → engine 为 null', () => {
        expect(createRequest().engine).toBe(null);
    });
});

describe('facade 拦截器 (my)', () => {
...
describe('facade createRequest 工厂 (my)', () => {
