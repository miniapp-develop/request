...
// 单入口 index，dual cjs+esm。@mini-dev/request 为普通依赖（workspace:*），构建时外置
// require('@mini-dev/request')，不内联打包——与核心 koa-compose 同策略，便于按平台独立发布。
// cjs pass 在 onSuccess 把 main 与 dependencies（协议解析为实版本）透传进 dist/cjs/package.json，
// 让小程序「构建 npm」运行时能跨包 require 到 @mini-dev/request 与 koa-compose。
