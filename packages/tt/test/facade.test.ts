...
describe('createRequest facade (tt)', () => {
    it('无 engine 时 handle 抛 No engine found（无 tt 环境）', async () => {
    ...
    it('createRequest() 缺省懒绑 tt 全局（设置 global.tt）', async () => {
        // 抖音 success res = { data, statusCode }（官方文档未列 header/cookies）
        const fakeVendor = {
            request(option: any) {
                option.success({ data: 'from-tt', statusCode: 200 });
                return { abort() {} };
            }
        };
        (global as any).tt = fakeVendor;
        try {
            const req = createRequest();
            expect(req.engine).not.toBeNull();
            const res = await req({ url: 'u' });
            expect(res.data).toBe('from-tt');
        } finally {
            delete (global as any).tt;
        }
    });

    it('createRequest() 无 tt 无 engine → engine 为 null', () => {
        expect(createRequest().engine).toBe(null);
    });
});

describe('facade 拦截器 (tt)', () => {
...
describe('facade createRequest 工厂 (tt)', () => {
