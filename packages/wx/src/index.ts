...
export { createWxEngine, createRequest, isTransientError };

/** wx 默认便捷单例：旧版 `@mini-dev/request` 的用户态 API（Promise + 两段式拦截器；库控制走 `ext`）。造独立实例 / 注入 engine 用 `createRequest(engine?)`。 */
