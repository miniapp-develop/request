// @mini-dev/request 抖音小程序示例（便捷层 + augment-only 两条 API 路径）
//
// 平台由独立包选定（@mini-dev/request-tt），不探测；import 不会抛错。
// 便捷层 API：Promise + 两段式拦截器 + 取消 + 超时 + 重试 + 缓存（库控制统一走 ext）。
// augment-only：直接用核心 createMethod + createTtEngine，原生入参/响应/错误原样透传，方法级拦截器是 koa 洋葱。
// 抖音 tt.request 原生不支持 chunked 流式，且 success 回调不暴露响应头（res.headers 为 undefined），
// 故 canonical 演示以服务端回显的请求头为准（res.data.headers），客户端 res.headers 不展示。
// 接口地址指向自带的 sample-server（http://127.0.0.1:8008），需先启动它。
import { request, createRequest, createMethod, createTtEngine, AbortController, RequestError, Response } from '@mini-dev/request-tt';
...
Page({
    ...
    // canonical headers 跨平台一致性：同一份复数 headers 写法，三平台都能正确发出。
    // tt 原生用 header（单数），engine 出向 canonical headers→header；服务端回显的请求头在 res.data.headers。
    ...
    async onTapMwShortCircuit() {
        const method = createMethod('request', { engine: createTtEngine });
    ...
    async onTapMwRetryDelay() {
        ...
        const method = createMethod('request', { engine: createTtEngine, retryDelay: customDelay });
